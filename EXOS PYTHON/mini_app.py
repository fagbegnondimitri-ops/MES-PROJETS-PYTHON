...
while CHOIX>0 and CHOIX<8:
        ...
        def modifie_client():
            ...
            for client in CLIENTS:
                if recherche_nom.lower() ==client['ENTREPRISE'].lower() and recherche_contact.lower() == client['CONTACT'].lower() and recherche_e_mail.lower()== client['E_MAIL'] and recherche_tel.lower() == client['TEL'] and recherche_statut.lower()== client['STATUT']:
                    ...
                    client['DATE_DE_MODIFICATION']=str(date3)
                    i+=1
                if i==1 :
                    if client['INTERACTIONS']!=[]:
                        REPONSE=input("Voulez vous modifier une interaction?(oui/non)")
                        if REPONSE.lower()=="oui":
                            ANNEE=int(input("ancienne annee d'interaction:"))
                            mois=int(input("ancien mois d'interaction:"))
                            jour=int(input("ancien jour d'interaction:"))
                            recherche_type= input("ancien type d'interaction:")
                            recherche_note= input("ancienne note d'interaction:")
                            date5=str(date(ANNEE,mois,jour))
                            for interaction in client['INTERACTIONS']:
                                if recherche_type.lower()==interaction['TYPE'].lower() and recherche_note.lower() == interaction['NOTE'].lower() and date5== interaction['DATE']:
                                    interaction['TYPE']= input("Nouveau type d'interaction:")
                                    interaction['NOTE']= input("Nouvelle note d'interaction")
                                    ANNEE1=int(input("nouvelle annee d'interaction:"))
                                    mois1=int(input("nouveau mois d'interaction:"))
                                    jour1=int(input("nouveau jour d'interaction:"))
    ...
    if CHOIX==7:
        ...
        if __name__ == "__main__": 
            relance_client()
    with open("C:/Users/USER/Documents/MES-PROJETS-PYTHON/EXOS PYTHON/mini.json","w",encoding="utf-8") as json_file:
